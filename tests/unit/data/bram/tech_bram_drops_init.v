// tech_bram_drops_init.v
//
// NEGATIVE TEST FIXTURE -- deliberately wrong, never install this.
//
// It is 'architecture/z1070/bram/tech_bram_with_init.v' with the
// '#(.INIT(INIT))' dropped from every '_TECHMAP_REPLACE_' instance : the three
// modules still declare an 'INIT' parameter, so 'memory_libmap' happily
// attaches the memory contents to the cell, and then the techmap throws them
// away on the floor.
//
// That combination is the failure the BRAM init check exists to catch.  It is
// worth a fixture of its own because nothing else notices it : the memory maps,
// the netlist is well formed, place and route succeed, and the only symptom is
// a BRAM that comes up holding zeros.
//
// Declaring no 'INIT' parameter at all is a different, and much louder, bug --
